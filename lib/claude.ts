...
const TRANSLATION_MIRROR_RULE = `- "translation" field for NOUN entries: use the native language's article convention for the same definiteness category as the original. Three cases:
  ...
  • Indefinite original (Scandi en/ett/ei per Rule 34) → native's INDEFINITE form, NEVER the native's DEFINITE form. The Scandi indef article is the LEMMA marker (not a semantic "a/an") — mirror this category across natives: "en bild" → "a picture" (EN, not "the picture"), "ein Bild" (DE, not "das Bild"), "une image" (FR, not "l'image"), "una imagen" (ES, not "la imagen"), "un'immagine" (IT, not "l'immagine"), "uma imagem" (PT, not "a imagem"), "een afbeelding" (NL, not "de afbeelding"). Same for "ett hus" → "a house" / "ein Haus" / "une maison" / "una casa" / "una casa" / "uma casa" / "een huis" (never "the house" / "das Haus" / "la maison" / etc.). For Polish or Czech natives: bare (no articles exist).
