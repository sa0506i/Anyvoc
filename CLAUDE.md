...
2. **Rotation cadence is `ROTATION_INTERVAL_MS` (4 s).** The 5 s on-screen cap is a product requirement, so no single message should be visible longer than that. Two rotation shapes exist: `setRotating` walks a flat array sequentially and stops on the last entry (used for the short `FETCH_ROTATION` / `OCR_ROTATION` lead-ins); `setRotatingPools` walks a 2-D array of phase pools, advancing one phase per tick and picking a random unused message from the current pool (used for the long `LLM_PHASES` ride). The final pool stays active for every further tick — it never loops back to earlier phases, and it allows repeats only once its own pool is exhausted.
3. **`processSharedText` emits opaque event codes**, not strings — the `onProgress: (event: ShareProgressEvent) => void` callback currently supports `'llm-start'` (caller sets `LLM_PHASES` via `setRotatingPools`) and `'saving'` (caller sets `SAVING`). Keeping the event vocabulary separate from UI copy lets the message set change without touching the data layer.
4. **LLM-phase tone rules** (baked into the `LLM_PHASES` pools): no apologies, no "please wait" / "thanks for your patience", no meta-commentary about the wait being long. Phases 7+ flatter the user's text or frame the delay as thoroughness — never as a problem. If you add new pools, mirror this tone; a regression here is a UX bug, not a style preference.
5. **Entry-point conventions:** every caller must set an intro (`INTRO.{link,image,text}`) **before** any async work, optionally set an input-specific rotation (`FETCH_ROTATION` for links, `OCR_ROTATION` for images, none for text), then pass the event dispatcher to `processSharedText`, which takes over driving the overlay through the LLM and save phases. The final `shareStore.stop()` must run in a `finally` block so a thrown error still hides the overlay.
